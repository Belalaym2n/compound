...
class NotificationService {
  static Future<void> saveNotifications(
    ...
    if (existingJsonString != null) {
      try {
        ...
        existingNotifications =
            existingJsonList.map((jsonItem) =>
                NotificationModel.fromJson(jsonItem)).toList();
      } catch (e) {
    ...
    existingNotifications.map((notification) => notification.toJson()).toList();
  ...
  static Future<void> saveLastNotifications(
      List<NotificationModel> newNotifications) async {
    final prefs = await SharedPreferences.getInstance();
    List<Map<String, dynamic>> notificationsJson=newNotifications
        .map((e) => e.toJson()).toList();
    String jsonString=jsonEncode(notificationsJson);

    await prefs.setString('lastNotification', jsonString);
  }

   static Future<List<NotificationModel>> getLastNotifications() async {
    final prefs = await SharedPreferences.getInstance();

    // Retrieve the JSON string from SharedPreferences
    String? jsonString = prefs.getString('lastNotification');
    ...
    try {
      ...
      return jsonList.map((jsonItem) => NotificationModel.fromJson(jsonItem))
          .toList();
    } catch (e) {
